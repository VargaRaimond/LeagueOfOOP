...
public final class PlayerFactory {
    ...
    public Player createHero(final Character type, final Point position, final int id) {
        switch (type) {
            case('W'):
                return new Wizard(position, HeroSpecs.WIZARD_BASE_HP,
                        HeroSpecs.WIZARD_HP_SCALE, id);
            case('K'):
                return new Knight(position, HeroSpecs.KNIGHT_BASE_HP,
                        HeroSpecs.KNIGHT_HP_SCALE, id);
            case('P'):
                return new Pyromancer(position, HeroSpecs.PYRO_BASE_HP,
                        HeroSpecs.PYRO_HP_SCALE, id);
            case('R'):
