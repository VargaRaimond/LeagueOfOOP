                    ...
                    if (allPlayers.get(i).getCoordinates().equals(allPlayers.get(j).getCoordinates())) {
                        if(allPlayers.get(i).getCurrentHp() > 0 && allPlayers.get(j).getCurrentHp() > 0) {
                            if (allPlayers.get(i).type.equals(PlayerType.Wizard)) {
                                if (!allPlayers.get(j).stunned) allPlayers.get(i).takeDamage(allPlayers.get(j));
                                if (!allPlayers.get(i).stunned) allPlayers.get(j).takeDamage(allPlayers.get(i));
                            } else {
                                if (!allPlayers.get(i).stunned) allPlayers.get(j).takeDamage(allPlayers.get(i));
                                if (!allPlayers.get(j).stunned) allPlayers.get(i).takeDamage(allPlayers.get(j));
                            }
                            ...
                            if (allPlayers.get(j).currentDotDuration == 0) {
                                allPlayers.get(j).stunned = false;
                            }
